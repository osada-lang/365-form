...
【執筆のルールと構成（厳格に以下の3セクションで記述してください）】

1. **第一セクション：診断結果と機会損失の可能性（※提案や解決策はここでは書かず、純粋な現状と機会損失のみを伝える）**:
   - セクションタイトル： **💡 【診断結果】${diagData.name}様の「デジタル店舗情報」の現状と、機会損失の可能性**
   - 評価されている実際の強み（★${diagData.rating.toFixed(1)}という高い顧客満足度や丁寧な接客・技術など）をまず客観的に称えてください。
   - そのうえで、総合スコア（${diagData.totalGained}/${diagData.totalPossible}点、${diagData.normalizedScore}%）と、現在設定が不足している項目（営業時間、説明文、写真枚数、投稿更新、返信率など）の事実を伝えてください。
   - **【重要】** ここでは改善案や提案、具体的な解決方法は一切書かず、「Web上で店舗を探す新規顧客が、情報不足や最新投稿の停止によって、競合他社へ流れてしまっている潜在的な機会損失（もったいない状態）」の事実のみを明確に伝えてください。

2. **第二セクション：集客最大化のために取り組むべき対策と「リソースの壁」（※まだ『365ボイス』の名前は出さない）**:
   - セクションタイトル： **🚀 競合と差をつけ集客を最大化する対策と、店舗運営における「リソース」の課題**
   - 機会損失を防ぎ、今より大きく集客を伸ばすために「本来行った方が良い対策と期待できる効果」を説明してください。
   - **【必須掲載のデータファクト】**: 『Googleマップにおいて、クチコミ件数が約300件に達するまでは、クチコミが50件増えるごとに問い合わせ・来店数が約1.2倍〜1.5倍に増加するという実証データがあります』というファクトを盛り込み、クチコミを増やすことや定期投稿・100%返信を行うことの絶大な集客効果を伝えてください。
   - **【店舗オーナーの心理形成】**: 「これを実行すれば間違いなく集客が大きく伸びる（やってみたい！）」と感じさせると同時に、「しかし、日々の営業をこなしながら、写真撮影・投稿作成・丁寧なクチコミ返信を手作業で毎日継続するのは、オーナー様やスタッフ様の時間的・体力的な負担（リソースの壁）が極めて大きく、現実的に非常に大変である」という課題意識・共感を提示してください。
   - ※注意：このセクション内では『365ボイス』の商品名は一切出さないでください。

3. **第三セクション：最小限の労力で成果を最大化する『365ボイス』のご提案**:
   - セクションタイトル： **🤝 本業に集中しながら最小限の労力で成果を最大化する『365ボイス』のご提案**
   - セクション２で浮き彫りになった「労力と時間の壁」を、オーナー様の負担を最小限に抑えて解決する手段として『365ボイス（365VOICE）』を提案します。
   - 365ボイスが「Googleマップ運用・クチコミ獲得・AI返信・LINE連携を最小限の作業で全自動化・仕組み化するシステム」であることをサラッと解説してください。
   - そのうえで、診断結果の数値から**この店舗に特に適した機能を【2つまたは3つだけ】ピックアップ**して、その魅力と解決策を具体的に提示してください。
     * 【選択基準】：
       - クチコミ不足（50件未満）➡ 『会計時やサービス提供後の満足度が高いタイミングで、無理なく自然に高品質なオンラインクチコミを獲得・集積する仕組み』
       - 返信率低下（70%未満または0%）➡ 『AIが感謝と集客キーワードを網羅した返信文章を即座に自動生成し、ワンクリックで返信対応を完了させる仕組み』
       - リピーター育成や広告費削減 ➡ 『Googleマップからの新規客を公式LINEへ自動誘導し、継続的なリピートファンへ育成する仕組み』
       - スタッフ接客業態 ➡ 『スタッフごとのクチコミ貢献度を可視化し、モチベーション向上とファン育成を両立する仕組み』

4. **締めの一言（営業担当へのバトンタッチ）**:
   - 末尾に必ず次の文章をそのまま掲載してください：
     「※貴店に合わせた具体的な活用方法や他店舗様での成功事例につきましては、本日お伺いしている営業担当より詳しくお伝えさせていただきます。」

以下の3つの構成タイトルで正確に出力してください：
1. 💡 【診断結果】${diagData.name}様の「デジタル店舗情報」の現状と、機会損失の可能性
2. 🚀 競合と差をつけ集客を最大化する対策と、店舗運営における「リソース」の課題
3. 🤝 本業に集中しながら最小限の労力で成果を最大化する『365ボイス』のご提案`;
