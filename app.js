...
以下のGBP診断レポート結果に基づき、店舗のオーナー様・店長様が直接読まれて「自分の店の改善点と、集客を高める仕組みがよく分かる」親身でプロフェッショナルな診断アドバイス文章を作成してください。

※重要：売り込みや営業色を前面に出しすぎず、あくまで客観的な「診断レポート」として、店舗様に寄り添った提案トーンを徹底してください。
...
【執筆のルールと全体のストーリー展開】
1. **診断としての客観的な改善点の提示**:
   - 診断データに基づき、この店舗で現在不足している・取り組むべき「改善点」を具体的に提示してください。
   - すべての土台として、まずは「GBP（Googleビジネスプロフィール）で現在足りていない情報の整理・登録」を完璧に整えることが最優先ステップ（スタートライン）であることをハッキリと伝えてください。
   - さらに、集客を最大化するための重要施策として「画像付きの毎日投稿」を行い、店舗の新鮮な最新情報を発信することで、新規顧客が「ここなら安心」「今度行ってみたい」と来店しやすくなる状態をつくる重要性を説明してください。

2. **改善によるメリットと理由の解説**:
   - 各改善点（情報の整理、毎日投稿、クチコミの獲得など）に対して、「こうすれば、こういう理由で、もっとWEB集客できる状態（検索上位や魅力が伝わる状態）になりますよ」という具体的な効果とロジックを分かりやすく解説してください。

3. **店舗運営における現実的な難しさへの共感**:
   - 上記の素晴らしい施策について解説したうえで、「しかし、これらすべての施策（毎日投稿、クチコミ促進、丁寧な返信、LINE連携など）を、毎日の店舗運営・接客を行いながら手動で継続していくのは、非常に大変で、現実的には『手が回らない（難しい）』と言っても過言ではないですよね？」と、オーナー様・店長様の日々の忙しさに深く共感してください。

4. **パートナーとしての『365ボイス』の自然な提示**:
   - 共感した後に、「もしご希望があれば、私たちはその忙しさを最小限に抑えつつ、MEO集客を最大化する力になることができます。それが、私たちが提供している『365ボイス（365VOICE）』という集客システムです」と自然に紹介してください。

5. **365ボイスがこの店舗のどう力になれるかの説明**:
   - 365ボイスが、この店舗の課題をどう解決できるのかを、以下の5つのポイント（店舗の状況に合わせた強み）を交えて優しく簡潔に説明してください：
     ① アンケートや抽選体験から、来店客にシステマチックにクチコミを促して自然に増やす仕組み
     ② 集まったクチコミに対し、MEO対策に効果的なキーワードを含む返信文を自動で作成・提案する仕組み
     ③ GBPと公式LINEを連携させ、来店からクチコミ、そしてLINEでの「長期的なファン（リピーター）」へと育てる仕組み
     ④ スタッフごとにクチコミ獲得数を管理し、スタッフの適正評価に繋げ、店舗だけでなく「スタッフのファン」も増やす仕組み
     ⑤ 単発で終わるその場しのぎの広告とは異なり、MEO（新規獲得）× LINE（ファン獲得）という「長期的に集客資産が積み上がる投資型システム（両輪の集客）」であること

6. **営業担当へのスムーズなトスアップ**:
   - 文章の最後には、売り込むのではなく、「※貴店での具体的な活用方法や他店舗様での成功事例につきましては、本日ご案内の営業担当より詳しくお伝えさせていただきます。」と添えて、自然に会話を引き継げる形で締めくくってください。

以下の3つの構成で出力してください（マークダウンの箇条書きや段落を活用し、読みやすく整形してください）：
1. 💡 店舗様の現状と改善点（GBP不足情報の整理を最優先とすること、画像付き毎日投稿の新規獲得効果を含む）
2. 🚀 改善による成果と、店舗運営における『現実的な難しさへの共感』
3. 🤝 私たちが力になれること：『365ボイス』による自動化・両輪の支援（5つの強みと長期投資としての説明、および営業への引き継ぎ）`;
