...
※※※【最重要：執筆の方向性とトーン】※※※
- **専門用語やロジックは徹底的に避ける**: 「MEO」「アルゴリズム」「評価ウェイト」「インデックス」といった専門的な専門用語や、小難しいSEOロジックは一切使わないでください（店舗オーナー様は、目の前のお客様を満足させることに命をかけている職人肌のプロであり、デジタル用語には不慣れな方がほとんどです）。
- **ロジカルよりも「共感・フィーリング・感情」で伝える**: 数字やランキングではなく、「お客様の気持ち」や「お店のあたたかさ」にフォーカスし、直感的に「あ、確かにそれって大事だな」と思える優しい比喩や表現を使ってください。
- **詳細なロジックは営業担当に任せる**: この文章ですべてを説明しようとせず、心に響く「気づき」と「共感」を与えることに集中してください。
...
【執筆のルールと全体のストーリー展開（フィーリング重視）】
1. **お店の『デジタル上の玄関』を整える大切さ（現状と改善点）**:
   - 診断データをもとに、不足している情報がある現状を「あたたかい比喩」で伝えてください。
     * 例：「せっかく素敵なお店なのに、ネット上の『玄関（マップ情報）』に看板がなかったり、営業時間という『のれん』が少しズレてしまっているような状態です。まずはネットの玄関をきれいに整えて、お客様が迷わず入れるようにすることからスタートしましょう」
   - **画像付きの毎日投稿の大切さ**:
     * 例：「お店の前の黒板に『今日のイチオシ』が毎日違う写真付きで書かれていると、前を通るお客様は『あ、今日行ってみよう！』とワクワクしますよね。ネット上でも同じように、画像付きで『今日の様子』を届けることで、初めてのお客様が安心して扉を開けやすくなります」

2. **集客できる理由と、日々の忙しさへの120%の共感**:
   - ネットの玄関を整え、発信を続けることで、「お店のあたたかさ」がそのままネットに広がり、新しいお客様に選ばれるようになります。
   - そのうえで、オーナー様・店長様の日々の頑張りに深く寄り添ってください：
     * 例：「でも、目の前のお客様に最高の料理やサービスを届けるために、毎日全力で立ち回っているなかで、スマホを片手に『毎日投稿を作る』『クチコミの返信を考える』『LINEを連動させる』といったネットの作業をやり続けるのは、体力的にも時間的にも『本当に大変で、現実的ではない』のが本音ですよね。目の前のお客様に100%集中したいのに、ネットのことにまで手が回らないのは、当然のことです」

3. **あたたかい裏方（サポーター）としての『365ボイス（365VOICE）』**:
   - 「もしよろしければ、その『ネット上の接客や玄関づくり』を、オーナー様に代わって陰でそっと自動でお手伝いするシステムがあります。それが『365ボイス』です」と優しく提示してください。
   - 365ボイスがどう力になれるかを、以下の5つの「あたたかいサポート内容」として直感的に説明してください：
     ① お店に来て喜んでくれたお客様の「美味しかった！」「ありがとう！」の生声を、簡単なアンケート感覚でネット上の『あたたかいご紹介（クチコミ）』として自然に集めてくれる仕組み
     ② いただいたクチコミに対して、感謝の気持ちとお店のこだわりがしっかり伝わる返信文を、AIが自動で優しく作成・提案してくれる仕組み
     ③ Googleマップで出会ったお客様と公式LINEで繋がり、お友達として「これからも末長く愛されるファン（常連さん）」へと育んでいく仕組み
     ④ 頑張っているスタッフさんの「お客様を笑顔にした数（クチコミ獲得数）」をハッキリと見える化し、スタッフさんのモチベーションや、その人個人のファンを増やすことにも繋がる仕組み
     ⑤ チラシやその場しのぎの広告を配り続けるのではなく、一度作ればずっと新規のお客様と常連様を呼び呼び込み続けてくれる、お店の「一生ものの集客システム（両輪）」になること

4. **営業担当者への引き継ぎ**:
   - 売り込むのではなく、「※貴店に合わせた具体的な活用方法や他店舗様での成功事例につきましては、本日お伺いしている営業担当より詳しくお伝えさせていただきます。」と添えて、自然な会話のトスアップで締めくくってください。

以下の3つの構成で出力してください（専門用語は一切使わず、絵文字や段落を効果的に使い、お便りのような親しみやすいマークダウン形式で出力してください）：
1. 💡 ネット上の『玄関』と『のれん』の現状（GBP情報整理と、ワクワクさせる毎日投稿の重要性）
2. 🚀 集客できる理由と、日々の店舗運営に対する『現実的な難しさへの共感』
3. 🤝 私たちが陰で支える力になります：あたたかい集客サポーター『365ボイス』（5つのあたたかい仕組みと長期的なファン作り、営業担当へのバトンタッチ）`;
